...
import { normalizeWhatsAppPhone, rememberPhoneShare } from '../../lib/utils.js'
import Pino from 'pino'

// ── In-memory session store ──────────────────────────────────────────────────
const sessions = new Map<string, WASocket>()
const readySessions = new Set<string>()
const connectingDevices = new Set<string>()   // prevent duplicate connect calls

const BROWSER_POOL: [string, string, string][] = [
...
export async function connectDevice(deviceId: string, orgId: string): Promise<void> {
  ...
  try {
    ...
    sock.ev.on('chats.phoneNumberShare', async ({ lid, jid }) => {
      const resolvedPhone = rememberPhoneShare(lid, jid)
      const legacyPhone = normalizeWhatsAppPhone(lid)

      if (!resolvedPhone || !legacyPhone || resolvedPhone === legacyPhone) {
        return
      }

      const legacyPhoneLid = `${legacyPhone}@lid`
      ...
      try {
        const { data: contacts, error } = await supabase
          ...
          .eq('org_id', orgId)
          .is('deleted_at', null)
          .in('phone', [legacyPhone, legacyPhoneLid, resolvedPhone])

        if (error || !contacts?.length) return

        const resolvedContact = contacts.find((contact) => contact.phone === resolvedPhone) || null
        const legacyContacts = contacts.filter((contact) => contact.phone !== resolvedPhone)
        ...
        if (resolvedContact) {
          await supabase
            ...
            .eq('id', resolvedContact.id)
            .eq('org_id', orgId)

          for (const legacyContact of legacyContacts) {
            await supabase
              .from('messages')
              .update({ contact_id: resolvedContact.id })
              .eq('org_id', orgId)
              .eq('contact_id', legacyContact.id)
            ...
            if (!existingConversation) {
              await supabase
                .from('conversations')
                .update({ contact_id: resolvedContact.id, updated_at: now })
                .eq('org_id', orgId)
                .eq('device_id', deviceId)
                .eq('contact_id', legacyContact.id)
            }
        ...
        logger.warn({ error, deviceId, orgId, lid, jid }, 'Unable to sync WhatsApp phone share identity')
      }
    })
      ...
      for (const update of updates) {
        ...
        try {
          ...
          try {
            ...
            // Non-critical: status sync can be retried from later events.
          }
        } catch { /* non-critical */ }
      }
    })
...
export function getSession(deviceId: string): WASocket | undefined {
  if (!readySessions.has(deviceId)) return undefined
  return sessions.get(deviceId)
}

export function getAllSessions(): Map<string, WASocket> {
  const activeSessions = new Map<string, WASocket>()
  for (const [deviceId, sock] of sessions.entries()) {
    if (readySessions.has(deviceId)) {
      activeSessions.set(deviceId, sock)
    }
  }
